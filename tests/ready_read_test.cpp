  ...
  public:
    ...
    void start() {
        result_ = std::async(std::launch::async, [this] {
            tgcli::daemon::ReadyReadSession reads(
                *client_, *session_,
                {.now = [this] { return clock_.now(); }, .wait = [this] { barrier_.wait(); }});
    ...
    void push_response(Clock::time_point observed_at) {
        ...
        runtime_->push_response(td_client_, sent.back().query_id,
                                tgcli::core::TdValue::from(tgcli::core::TdOk{}), std::nullopt,
                                observed_at);
    }

    void push_auth(tgcli::core::AuthState state, Clock::time_point observed_at) {
        runtime_->push_update(td_client_, {}, tgcli::core::AuthStateData{state}, observed_at);
    }
...
TEST_CASE("Ready read uses event observation time at the absolute deadline",
    ...
    SECTION("response observed before the deadline remains eligible when consumed late") {
    ...
    SECTION("authorization loss observed before the deadline remains eligible when consumed late") {
        ReadyReadHarness harness(deadline);
        harness.start();
