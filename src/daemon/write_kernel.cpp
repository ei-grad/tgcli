...
WriteKernelResult audit_fatal() {
    return {WriteKernelStatus::AuditFatal, std::nullopt, std::nullopt};
}

json timeout_terminal(proto::M3Operation operation) {
    return error_terminal("TIMEOUT", "request timed out",
                          {{"operation", operation_name(operation)}, {"state", nullptr}}, kTimeout);
}
...
WriteKernelResult gate_failure(const WriteKernelRequest& request,
    ...
    if (gate.status == IdempotencyCoreGateStatus::Interrupted) {
        ...
        return interruption == AccountAuditFailure::Interruption::Deadline
                   ? rejected(timeout_terminal(request.operation))
...
json pending_terminal(const WriteKernelRequest& request, const IdempotencyEntry& incumbent,
    ...
    return error_terminal("IDEMPOTENCY_PENDING",
                          ...
                          {{"operation", operation_name(request.operation)},
                           ...
                           {"fingerprint", request.request_fingerprint.value()},
...
json conflict_terminal(const WriteKernelRequest& request, const IdempotencyEntry& incumbent,
    ...
    return error_terminal("IDEMPOTENCY_CONFLICT",
                          ...
                          {{"operation", operation_name(request.operation)},
                           ...
                           {"actual_fingerprint", request.request_fingerprint.value()}},
                          kUsage);
}
...
WriteKernelResult confirm_replay(const WriteKernelRequest& request, const WriteKernelHooks& hooks,
    ...
    if (destructive(request.operation)) {
        ...
        const auto confirmation = hooks.confirm(*plan, true);
...
std::optional<WriteKernelResult> incumbent_result(const WriteKernelRequest& request,
                                                  const WriteKernelHooks& hooks,
                                                  const IdempotencyLookup& lookup,
                                                  const IdempotencyKeyHash& key_hash) {
    ...
    if (lookup.status == IdempotencyLookupStatus::Pending) {
        return WriteKernelResult{WriteKernelStatus::Pending,
                                 pending_terminal(request, *lookup.incumbent, key_hash),
                                 std::nullopt};
    }
    if (lookup.status == IdempotencyLookupStatus::Conflict) {
        return WriteKernelResult{WriteKernelStatus::Conflict,
                                 conflict_terminal(request, *lookup.incumbent, key_hash),
                                 std::nullopt};
    }
...
std::optional<json> config_cas_terminal(const WriteKernelRequest& request,
    ...
    case config::GrantVerificationStatus::TimedOut:
        return timeout_terminal(request.operation);
...
WriteKernelResult WriteKernel::run(const WriteKernelRequest& request,
    ...
    if (!operation || request.arguments.operation() != request.operation ||
        request.request_source_bytes == 0 ||
        request.request_source_bytes > proto::kMaximumRequestSourceBytes ||
        request.config_path.empty() || !hooks.plan) {
        return audit_fatal();
    }

    if (request.dry_run) {
        auto planning = hooks.plan();
    ...
    const AccountAuditScanControl scan_control{request.deadline, request.cancelled};
    {
        ...
        const auto gate = foundation_->run_core_gate(epoch, request.sampled_now);
        ...
        if (request.idempotency_key_hash) {
            const auto lookup = IdempotencyStore::lookup(
                gate.snapshot, *request.idempotency_key_hash, request.request_fingerprint);
            if (auto result =
    ...
    if (request.idempotency_key_hash) {
        const auto lookup = IdempotencyStore::lookup(gate.snapshot, *request.idempotency_key_hash,
                                                     request.request_fingerprint);
        if (auto result = incumbent_result(request, hooks, lookup, *request.idempotency_key_hash)) {
            return std::move(*result);
        }
    }
    ...
    if (destructive(request.operation)) {
        ...
        const auto confirmation = hooks.confirm(proposed_plan, false);
        if (confirmation.status != WriteConfirmationStatus::ConfirmedYes &&
            confirmation.status != WriteConfirmationStatus::ConfirmedTty) {
            return {WriteKernelStatus::Rejected, confirmation.terminal, std::nullopt};
        }
    ...
    auto intent = make_account_audit_intent(
         ...
         *operation,
         ...
         proposed_plan.value(),
         request.request_fingerprint.value(),
    ...
    if (request.idempotency_key_hash) {
        ...
        auto pending = make_idempotency_pending_entry(
            {*request.idempotency_key_hash, request.request_fingerprint, *operation,
             request.invocation_id, receipt.audit_generation, request.sampled_now,
             proposed_plan.value()},
        ...
        if (inserted.status == IdempotencyInsertStatus::UnexpectedIncumbent) {
            ...
            return {WriteKernelStatus::DurabilityFatal, std::move(closure.terminal), std::nullopt};
        }
        ...
        if (!append_checkpoint(*foundation_, epoch, request, hooks, *operation,
                               ...
                               {{"key_hash", request.idempotency_key_hash->value()},
                                {"request_fingerprint", request.request_fingerprint.value()},
    ...
    if (hooks.post_intent) {
        post_intent = hooks.post_intent(proposed_plan);
    }
    if (post_intent.spool) {
        if (request.operation != proto::M3Operation::SavedAttach ||
            !validate_account_audit_persisted_spool(*post_intent.spool, request.invocation_id) ||
        ...
        if (request.idempotency_key_hash) {
            const auto updated = foundation_->store().update_spool(
                *request.idempotency_key_hash, request.invocation_id, *post_intent.spool, epoch);
            if (updated.status == IdempotencyWriteStatus::Failed) {
                return audit_fatal();
            }
        }
        current_spool_hold = foundation_->audit().hold_current_spool(receipt, *post_intent.spool,
                                                                     epoch, audit_failure);
    ...
    const auto cleanup_current_spool = [&](bool clear_completed_store) {
        ...
        auto cleanup = hooks.cleanup_spool
        ...
        if (clear_completed_store && request.idempotency_key_hash) {
            const auto cleared = foundation_->store().clear_spool(*request.idempotency_key_hash,
    ...
    if (post_intent.terminal_without_dispatch) {
        const auto& terminal = *post_intent.terminal_without_dispatch;
        ...
        if (request.idempotency_key_hash) {
            const auto removed = foundation_->store().remove_owned(*request.idempotency_key_hash,
                                                                   request.invocation_id, epoch);
            if (removed.status == IdempotencyWriteStatus::Failed) {
                return audit_fatal();
            }
        }
        if (!cleanup_current_spool(false)) {
            return audit_fatal();
        }
        return {WriteKernelStatus::Completed, terminal.value(), proposed_plan};
    }

    if (!hooks.prepare_dispatch || !hooks.dispatch) {
        return audit_fatal();
    }
    const auto dispatch_preparation = hooks.prepare_dispatch(proposed_plan);
    if (!append_checkpoint(*foundation_, epoch, request, hooks, *operation, ++checkpoint_sequence,
                           AccountAuditStage::DispatchStarted, dispatch_preparation.proof,
                           completed)) {
        return audit_fatal();
    }
    const auto dispatch = hooks.dispatch(proposed_plan);
    if (dispatch.terminal.operation() != request.operation ||
        !write_contract::terminal_matches_plan(dispatch.terminal, proposed_plan)) {
        return audit_fatal();
    }
    if (dispatch.temporary_message_ids) {
        if (!append_checkpoint(*foundation_, epoch, request, hooks, *operation,
                               ++checkpoint_sequence, AccountAuditStage::TemporaryIdsObserved,
                               {{"temporary_message_ids", *dispatch.temporary_message_ids}},
                               completed)) {
            return audit_fatal();
        }
        if (request.idempotency_key_hash) {
            const auto updated = foundation_->store().update_temporary_message_ids(
                *request.idempotency_key_hash, request.invocation_id,
                *dispatch.temporary_message_ids, epoch);
            if (updated.status == IdempotencyWriteStatus::Failed) {
                return audit_fatal();
            }
        }
    }
    if (dispatch.forward_progress) {
        if (!append_checkpoint(*foundation_, epoch, request, hooks, *operation,
                               ++checkpoint_sequence, AccountAuditStage::ForwardProgress,
                               {{"items", *dispatch.forward_progress}}, completed)) {
            return audit_fatal();
        }
        if (request.idempotency_key_hash) {
            const auto updated = foundation_->store().update_forward_progress(
                *request.idempotency_key_hash, request.invocation_id, *dispatch.forward_progress,
                epoch);
            if (updated.status == IdempotencyWriteStatus::Failed) {
                return audit_fatal();
            }
    ...
    if (dispatch.mutation_confirmed &&
        !append_checkpoint(*foundation_, epoch, request, hooks, *operation, ++checkpoint_sequence,
    ...
    if (request.idempotency_key_hash) {
        ...
        if (dispatch.mutation_state == AccountAuditMutationState::None) {
            ...
            transition =
                foundation_->store().complete(*request.idempotency_key_hash, request.invocation_id,
    ...
    if (dispatch.mutation_state != AccountAuditMutationState::Possible &&
        !cleanup_current_spool(dispatch.mutation_state == AccountAuditMutationState::Confirmed)) {
        return audit_fatal();
    }
    if (hooks.cleanup && !hooks.cleanup(proposed_plan, dispatch)) {
        return audit_fatal();
    }
