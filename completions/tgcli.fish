...
complete -c tgcli -f -n 'test (__tgcli_state (commandline -opc)) = '\''||0'\''' -a 'account chat chats completion contact daemon doctor fetch folder history listen login logout me msg read resolve saved schema send session storage topic unread version wait-for'
complete -c tgcli -f -n 'test (__tgcli_state (commandline -opc)) = '\''account||0'\''' -a 'add list remove show use'
complete -c tgcli -f -n 'test (__tgcli_state (commandline -opc)) = '\''chat||0'\''' -a 'archive ban demote invite-link join kick leave mark-read mute pin promote set-description set-permissions set-photo set-title unarchive unban unmute unpin'
...
complete -c tgcli -f -n 'test (__tgcli_state (commandline -opc)) = '\''completion||0'\''' -a 'bash zsh fish --json --verbose -v --no-daemon --no-color'
